...
def train():
    ...
    images, labels = get_data_set(width=img_x, height=img_y, nr_of_chars=nr_of_chars, color=color,
                                  nr_of_captchas=numberOfCaptchas, number=numbers)
    ...
    model.add(Conv2D(16, (5, 5), padding='same', input_shape=(img_x, img_y, 1), activation="relu"))
    ...
    if not numbers:
        ...
        model_dir = os.path.join(output_dir, 'model_nrs' + str(nr_of_chars) + '_acc_' + str(max(val_acc)).replace('.', '_')[:4])
