...
def get_all_possible_label_categories(nr_of_chars, numbers=False):
    if not numbers:
...
def get_data_set(width=64, height=64, color=False, nr_of_chars=1, nr_of_captchas=100, numbers=False):
    labels = []
    images = []
...
def main():
    images, labels = get_data_set(color=True, nr_of_captchas=1, nr_of_chars=2)
    print(labels[0])
    print(images[0])
    plt.imshow(images[0])
    plt.show()
